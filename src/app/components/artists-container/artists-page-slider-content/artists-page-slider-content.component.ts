...
@Component({
  selector: 'app-artists-page-slider-content',
  templateUrl: './artists-page-slider-content.component.html',
  styleUrls: ['./artists-page-slider-content.component.css']
})
export class ArtistsPageSliderContentComponent implements OnInit {
  ...
  onUploadOutput(output: UploadOutput): void {
    if (output.type === 'allAddedToQueue') { // when all files added in queue
      ...
      const event: UploadInput = {
        type: 'uploadAll',
        url: this.uploadUrl + 'artist-slider-photo.php?action=upload_artist_slider_image',
        method: 'POST',
        data: { foo: 'bar' }
      };
      this.uploadInput.emit(event);
  ...
  saveSliderPhotos() {
    ...
    const url = 'artist-slider-photo.php?action=update_slider_photo';
  ...
  getSliderPhoto() {
    ...
    this._commonRequestResponseService.post('artist-slider-photo.php?action=get_slider_photo', inputJson)
      .subscribe((res) => {
        console.log('api response--', res);
        if (res && res.length > 0) {
          this.loadSliderStatus = true;
          console.log('user_slider_images--', res);
          this.artistSliderPhotoData = res[0]['user_slider_images'];
        } else {
          this.artistSliderPhotoData = [];
        }
      }, (err) => {
        console.log('got Error', err);
      });
