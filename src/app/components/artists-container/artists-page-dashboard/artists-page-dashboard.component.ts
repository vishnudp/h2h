...
@Component({
  selector: 'app-artists-page-dashboard',
  templateUrl: './artists-page-dashboard.component.html',
  styleUrls: ['./artists-page-dashboard.component.css']
})
export class ArtistsPageDashboardComponent implements OnInit {
  ...
  startUpload(): void {
    const event: UploadInput = {
      type: 'uploadAll',
      url: this.uploadUrl + 'artist-profile-photo.php?action=upload_artist_profile_image',
      method: 'POST',
      data: { foo: 'bar' }
    };
  ...
  saveProfilePhoto() {
    ...
    const url = 'artist-profile-photo.php?action=update_profile_photo';
  ...
  getProfilePhoto() {
    ...
    this._commonRequestResponseService.post('artist-profile-photo.php?action=get_profile_photo', inputJson)
      .subscribe((res) => {
        console.log('api response--', res);
        if (res && res.length > 0) {
          ...
          if (res[0] && res[0]['user_logo_info'] && res[0]['user_logo_info'][0]) {
            this.artistProfilePhotoData = res[0]['user_logo_info'][0];
          }
        } else {
          this.artistProfilePhotoData = [];
        }
      }, (err) => {
        console.log('got Error', err);
      });
  ...
  deleteProfilePhoto() {
    ...
    this._commonRequestResponseService.post('artist-profile-photo.php?action=delete_profile_photo', inputJson)
      .subscribe((res) => {
        console.log('api response--', res);
        if (res) {
