...
function get_catalog_videos_title() {  
    $qry = mysql_query('SELECT distinct * from tg_catalogue where post_is_deleted = 0 and user_id = 1 and user_role_id = 1 and post_type = "video"');
    $data = array();
...
function get_catalog_videos_per_title() {  
  ...
  if ($data) {
    $post_id = $data->postId;
  $qry = mysql_query('SELECT distinct * from tg_catalogue where post_is_deleted = 0 and user_id = 1 and post_id = "'.$post_id.'" and user_role_id = 1 and post_type = "video"');
  $data = array();
  while($rows = mysql_fetch_array($qry))
  {
      $data[] = array(
