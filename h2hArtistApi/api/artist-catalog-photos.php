...
switch($_GET['action'])  {
    ...
    case 'upload_artist_catalog_images' :
    upload_artist_catalog_images();
            break;   
    case 'addPhotos':
          addPhotos();
          break; 
    case 'get_catalog_photos_title':
          get_catalog_photos_title();
          break;
    case 'get_catalog_photos_per_title':
          get_catalog_photos_per_title();
          break;
    case 'update_catalog_photos':
          update_catalog_photos();
          break;
    case 'delete_catalog_photo':
          delete_catalog_photo();
          break;
    case 'delete_catalog_photo_attachment':
          delete_catalog_photo_attachment();
          break;
...
function upload_artist_catalog_images() {
...
function get_catalog_photos_title() {  
    $qry = mysql_query('SELECT distinct * from tg_catalogue where post_is_deleted = 0 and user_id = 1 and user_role_id = 1 and post_type = "image"');
    $data = array();
...
function get_catalog_photos_per_title() {  
  $data = json_decode(file_get_contents("php://input")); 
  if ($data) {
    $post_id = $data->postId;
  $qry = mysql_query('SELECT distinct * from tg_catalogue where post_is_deleted = 0 and user_id = 1 and post_id = "'.$post_id.'" and user_role_id = 1 and post_type = "image"');
  $data = array();
  while($rows = mysql_fetch_array($qry))
  {
      $data[] = array(
...
function update_catalog_photos() {
  $data = json_decode(file_get_contents("php://input"));   
  if ($data) {
...
function delete_catalog_photo_attachment() {
  $data = json_decode(file_get_contents("php://input"));     
    $post_id = $data->post_id; 
    $user_id     = $data->user_id;
    $user_role_id      = $data->user_role_id;
